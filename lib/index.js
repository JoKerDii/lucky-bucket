...
const BALL_SIZE = 16;
const CANVAS_HEIGHT = 700; // 600
const CANVAS_WIDTH = 794;
const PEG_X = 70;
const PEG_Y = 70;
const BUCKET_COLOR = '#e1f5fe';
// const COLORS = [
//   '#e1f5fe',
//   '#b3e5fc',
//   '#81d4fa',
...
Events.on(engine, 'collisionActive', ({ pairs }) => {
  const filteredPairs = pairs.forEach(pair => {
    if (
      ...
      // stats
      document.querySelector('.x-bounciness').innerHTML = `Bounciness: ${Math.round(pair.bodyB.restitution * 10000) / 10000}`;
      document.querySelector('.x-ball-size').innerHTML = `Ball Size: ${Math.round(pair.bodyB.size * 10000) / 10000}`;
      document.querySelector('.x-position').innerHTML = `Drop Position: ${Math.round(pair.bodyB.dropX)}`;
      // end
...
document.querySelector('button#predict').addEventListener('click', runPrediction);
